...
export default function ResultsDisplay({ results }: ResultsDisplayProps) {
  const isSuccess = results.successful !== undefined ? results.successful > 0 : results.success
  const successCount = results.successful || results.successCount || 0
  const failureCount = results.failed || results.failureCount || 0
            ...
            {results.results && results.results.length > 0 && (
              <div className="mt-6">
                ...
                <div className="max-h-64 overflow-y-auto space-y-2">
                  {results.results.map((detail: any, index: number) => (
                    <div
                      key={index}
                    ...
                    >
                      <span className="font-medium">{detail.username_or_phone}</span>
                      {detail.message && (
                        <span className="text-xs ml-2">- {detail.message}</span>
                      )}
                    </div>
                  ))}
                </div>
